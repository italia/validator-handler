...
import { upload as s3Upload, empty as s3Delete } from "../controller/s3Controller"
    ...
    .then(async () => {
        ...
        for (let element of jobObjs) {
            ...
            if (uploadResult.status) {
                await successReport(jobObj, cleanJson, uploadResult.jsonLocationUrl, uploadResult.htmlLocationUrl)
            } else {
...
const successReport = async (jobObj, cleanJsonResult: string, jsonUrl: string, htmlUrl: string) => {
    await jobObj.update({
        status: 'PASSED',
        end_at: Date.now(),
...
const uploadFiles = async (jobObj: Job, htmlReport: string, jsonReport: string) : Promise<{ status: boolean; htmlLocationUrl: string | null; jsonLocationUrl: string | null }> => {
    try {
        const htmlLocationUrl = await s3Upload(htmlReport, jobObj.entity_id + '/' + jobObj.id + '/' + 'report.html')
